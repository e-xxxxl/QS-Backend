...
//     // First, check if parcel exists on Terminal Africa
//     let parcelExists = false;
    
//     try {
//       console.log(`🔍 Checking if parcel ${parcel_id} exists...`);
//       const parcelCheck = await axios.get(`${this.baseURL}/parcels/${parcel_id}`, {
//         headers: {
...
//       parcelExists = parcelCheck.data.success;
//       console.log(`✅ Parcel ${parcel_id} exists on Terminal Africa`);
      
//     } catch (parcelError) {
//       console.log(`❌ Parcel ${parcel_id} not found or error:`, parcelError.message);
//       parcelExists = false;
//     }
...
//     if (parcelExists) {
//       // Use parcel ID if it exists - THIS IS THE KEY FIX
//       terminalData = {
//         shipment: {
//           address_from: address_from_id,
//           address_to: address_to_id,
//           parcels: [parcel_id],  // JUST THE ID - not parcelData!
//           rates: [rate_id],
//           metadata: metadata,
//           async: false
//         }
//       };
//       console.log('📦 Using existing parcel ID:', parcel_id);
//     } else {
//       // Create parcel object from metadata only if parcel doesn't exist
//       console.log('⚠️ Parcel does not exist, creating from metadata...');
      
//       // Clean items array - remove any unwanted fields
//       const cleanItems = (metadata.items || []).map(item => ({
//         description: item.description || 'Item',
//         quantity: parseInt(item.quantity || 1),
//         value: parseFloat(item.value || 0),
//         currency: item.currency || 'NGN',
//         weight: parseFloat(item.weight || metadata.parcel_weight || 1)
...
//           address_from: address_from_id,
//           address_to: address_to_id,
//           parcels: [parcelData],  // Full parcel object
//           rates: [rate_id],
//           metadata: metadata,
//           async: false
//         }
//       };
//       console.log('📦 Using full parcel object (created from metadata)');
//     }

//     console.log('📦 Terminal Africa request data:', JSON.stringify(terminalData, null, 2));

//     // Make the API call
//     const response = await axios.post(`${this.baseURL}/shipments`, terminalData, {
//       headers: {
//         'Content-Type': 'application/json',
//         'Authorization': `Bearer ${this.apiKey}`,
//         'User-Agent': 'QuickShip/1.0'
//       },
//       timeout: 30000
//     });
...
//       // Try the alternative approach if parcel exists but still fails
//       if (parcelExists && response.data?.message?.includes('Parcel')) {
//         console.log('🔄 Parcel exists but API rejected it, trying alternative...');
//         return await this.tryAlternativeApproach(address_from_id, address_to_id, parcel_id, rate_id, metadata);
//       }
      
//       throw new Error(response.data?.message || 'Failed to create shipment on Terminal Africa');
//     }

//     console.log('✅ Terminal Africa shipment created successfully!');
//     const terminalShipment = response.data.data;
    
//     return {
//       success: true,
//       shipment_id: terminalShipment.id,
//       tracking_number: terminalShipment.tracking_number,
//       status: terminalShipment.status,
//       carrier: terminalShipment.carrier,
//       carrier_name: terminalShipment.carrier_name,
//       rate_id: terminalShipment.rate_id,
//       label_url: terminalShipment.label_url,
//       tracking_url: terminalShipment.tracking_url,
//       estimated_delivery: terminalShipment.estimated_delivery,
//       amount: terminalShipment.amount,
//       currency: terminalShipment.currency
//     };
...
// async tryAlternativeApproach(address_from_id, address_to_id, parcel_id, rate_id, metadata) {
//   try {
//     console.log('🔄 Trying alternative approach...');
    
//     // Approach 1: Create a new parcel first
//     const parcelData = {
//       weight: parseFloat(metadata.parcel_weight || 1),
//       length: parseFloat(metadata.parcel_length || 10),
//       width: parseFloat(metadata.parcel_width || 10),
...
//     const parcelResponse = await axios.post(`${this.baseURL}/parcels`, {
//       parcel: parcelData
//     }, {
//       headers: {
//         'Content-Type': 'application/json',
...
//     if (!parcelResponse.data.success) {
//       throw new Error('Failed to create parcel: ' + parcelResponse.data.message);
//     }
    
//     const newParcelId = parcelResponse.data.data.id;
//     console.log('✅ New parcel created:', newParcelId);
    
//     // Now create shipment with new parcel ID
//     const terminalData = {
//       shipment: {
//         address_from: address_from_id,
//         address_to: address_to_id,
...
//     console.log('📦 Creating shipment with new parcel...');
    
//     const shipmentResponse = await axios.post(`${this.baseURL}/shipments`, terminalData, {
//       headers: {
//         'Content-Type': 'application/json',
//         'Authorization': `Bearer ${this.apiKey}`
//       }
...
//       throw new Error('Failed to create shipment: ' + shipmentResponse.data.message);
//     }
    
//     const terminalShipment = shipmentResponse.data.data;
...
// Alternative format if first attempt fails
...
//         currency: terminalShipment.currency
//       };
//     } else if (response.data && response.data.message === 'Shipment created successfully') {
//       // FIX: Handle case where message says success but success flag might be missing
//       console.log('✅ Shipment created successfully (message indicates success)');
...
//       return {
//         success: true,
//         shipment_id: terminalShipment.shipment_id || terminalShipment.id || 'unknown',
//         tracking_number: terminalShipment.tracking_number || 'pending',
//         status: terminalShipment.status || 'processing',
//         carrier: terminalShipment.carrier || 'unknown',
//         carrier_name: terminalShipment.carrier_name || 'unknown',
//         rate_id: terminalShipment.rate_id || rate_id,
//         label_url: terminalShipment.label_url,
//         tracking_url: terminalShipment.tracking_url,
//         estimated_delivery: terminalShipment.estimated_delivery_date || terminalShipment.estimated_delivery,
//         amount: terminalShipment.amount || parseFloat(metadata.total_amount) || 0,
//         currency: terminalShipment.currency || 'NGN',
...
// In terminalAfricaService.js - Update the createShipmentAlternative function
