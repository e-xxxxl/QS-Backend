  ...
  try {
    ...
    if (shipmentData && metadata.userId === userId.toString()) {
      ...
      try {
        ...
        console.error(
          "❌ Error creating shipment after payment:",
          shipmentError,
        );
      ...
      data: {
        ...
        nextSteps: createdShipment
          ? [
              ...
              "Tracking number: " +
                (createdShipment.terminalShipmentId || "Will be assigned"),
...
exports.createShipmentWithPayment = async (req, res) => {
  try {
    const userId = req.user._id;
    const { shipment_data, payment_email } = req.body;
    ...
    const selectedRate = rates.find(
      (rate) => rate.rate_id === shipment_data.rate_id,
    );
  ...
  try {
    ...
    if (shipment_data && shipment_data.rate_id) {
      ...
      try {
        ...
        const originalAmount =
          shipment_data.metadata?.original_amount ||
          shipment_data.metadata?.total_amount ||
          paymentData.amount / 100;

        const serviceFeePercentage =
          shipment_data.metadata?.service_fee_percentage || 25;
        const serviceFeeAmount =
          shipment_data.metadata?.service_fee_amount || 0;

        // Handle estimated_delivery
        // Handle estimated_delivery - fix the "Invalid Date" issue
        let estimatedDelivery =
          shipment_data.metadata?.estimated_delivery ||
        ...
        if (estimatedDelivery) {
          try {
            ...
            if (
              ...
              estimatedDelivery.includes("Within")
            ) {
            ...
            console.warn(
              "⚠️ Date parsing error, using default:",
              dateError.message,
            );
          ...
          estimatedDelivery = new Date(Date.now() + 5 * 24 * 60 * 60 * 1000);
          console.log(
            "📅 Using default estimated delivery:",
        ...
        const shipment = new Shipment({
          ...
          sender: {
            ...
            email:
              shipment_data.metadata?.user_email || paymentData.customer?.email,
          ...
          parcel: {
            ...
            items: shipment_data.metadata?.items || [
              {
                description:
                  shipment_data.metadata?.item_description || "Package",
                quantity: parseInt(shipment_data.metadata?.item_quantity || 1),
                value: parseFloat(
                  shipment_data.metadata?.item_value || originalAmount,
                ),
          ...
          shipping: {
            carrier:
              terminalResponse.carrier ||
              shipment_data.metadata?.carrier ||
              "QuickShip",
            carrier_name:
              terminalResponse.carrier_name ||
              shipment_data.metadata?.carrier_name ||
              "QuickShip Carrier",
            service: shipment_data.metadata?.service || "Standard",
            rate_id: shipment_data.rate_id,
            amount: terminalResponse.amount || originalAmount,
            currency:
              terminalResponse.currency || paymentData.currency || "NGN",
            estimated_delivery:
              estimatedDelivery ||
        ...
        console.error(
          "🚨 CRITICAL: Payment successful, Terminal Africa shipment created, but DB save failed",
        );
        console.error(
          "Terminal Africa Shipment ID:",
          terminalResponse?.shipment_id,
        );
    ...
    if (createdShipment) {
      try {
        ...
        // Send emails
        try {
          await sendPaymentAndShipmentEmails(
            userId,
            {
              reference: paymentData.reference,
              amount: paymentData.amount / 100,
    ...
    res.status(200).json({
      ...
      message: createdShipment
        ? "Payment successful and shipment created"
        : "Payment successful",
      data: {
        payment: {
          reference: paymentData.reference,
